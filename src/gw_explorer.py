...
def _load_csv_numeric(path):
    """Load a CSV file, return (headers, columns) for numeric columns only.

    Raises ValueError on empty/invalid files.
    """
...
def _pick_from_list(parent, title, prompt, options):
    """Modal listbox dialog. Returns the chosen string or None on cancel."""
    ...
    dlg.grab_set()
    tk.Label(dlg, text=prompt, justify="left").pack(padx=12, pady=(10, 4), anchor="w")
    lb = tk.Listbox(dlg, height=min(14, len(options)), width=44,
    ...
    btn = tk.Frame(dlg)
    btn.pack(pady=(6, 10))
    tk.Button(btn, text="OK", command=on_ok, width=10).pack(side=tk.LEFT, padx=4)
    tk.Button(btn, text="Cancel", command=on_cancel, width=10).pack(side=tk.LEFT, padx=4)
...
class GWExplorer:
    ...
    def _build_spectrogram(self):
        ...
        filtered = bandpass(self.strain, 10, min(4000, sr / 2 - 1), sr)
        normed = normalize(filtered, target_peak=1.0)
        nperseg = min(512, max(32, len(normed) // 8))
        if nperseg >= len(normed):
            self.spec_surf = None
            return
        times, freqs, spec_db = compute_spectrogram(
            normed, sr, nperseg=nperseg, overlap_frac=0.75)
    ...
    def _build_widgets(self):
        ...
        y += 46
        self.btn_load = PgButton(cx + 5, y, cw - 10, 28, "Load Data...", self.small)
    ...
    def _load_csv_as_strain(self, path, root):
        """Load a CSV, prompt for column if multiple, return (strain, info).

        Returns None if the user cancels the column picker.
        """
        headers, columns = _load_csv_numeric(path)
        if len(headers) == 1:
            col_name = headers[0]
        else:
            col_name = _pick_from_list(
                root, "Pick column",
                f"{os.path.basename(path)} has {len(headers)} numeric columns.\n"
                f"Pick one to use as the strain signal:",
                headers)
            if col_name is None:
                return None
        data = np.nan_to_num(columns[col_name], nan=0.0).astype(np.float64)
        samples = len(data)
        # Treat each data point as 10ms — matches the csv_viewer convention.
        sample_rate = 100
        duration = samples / sample_rate
        info = {
            "detector": f"CSV[{col_name}]",
            "utc_start": os.path.basename(path),
    ...
    def _on_load(self):
        ...
        try:
            path = filedialog.askopenfilename(
                ...
                filetypes=[
                    ("All supported", "*.hdf5 *.h5 *.hdf *.csv"),
                    ("HDF5 files", "*.hdf5 *.h5 *.hdf"),
                    ("CSV files", "*.csv"),
                    ("All files", "*.*"),
                ],
            ...
            try:
                if ext in (".hdf5", ".h5", ".hdf"):
                    ...
                    result = self._load_csv_as_strain(path, root)
                    if result is None:
                ...
                self._build_spectrogram()
                self.status = (
                    f"Loaded: {info['detector']} | {info['samples']:,} samples")
...
def main():
    ...
    dev = 13
    server = Server(sr=PLAYBACK_SR, nchnls=2, duplex=0, buffersize=4096,
                    audio="portaudio")
